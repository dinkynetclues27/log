    ...
    <div className="container start-container">
      <div className="row justify-content-center">
        <div className="col-md-6">
          ...
          {showSuccess &&(<div style={{backgroundColor:"#90EE90"}}>
            <h3>Success data</h3>
            <ul>
            {success.map((successs, index) => (
                  <li key={index}>{successs.vendor_name} successfull on {successs.created_at}</li>
                ))}
            </ul>
          </div>)}
          {showFail &&(<div style={{backgroundColor:'#FF7F7F'}}>
            <h3>Fail data</h3>
            <ul>
            {fail.map((fails, index) => (
                  <li key={index}>{fails.vendor_name} successfull on {fails.created_at}</li>
                ))}
            </ul>
          </div>)}
                {showLogs && (
